...
| 测试 | **618 passed**（本机 8~26 秒；全离线、不碰真实数据，`test_reader.py` 199 项 / `test_config.py` 50 项） |
| 类型检查 | `npx pyright` → **0 errors, 0 warnings, 0 informations**（`wreader/`、`tests/`、`tools/` 都纳入） |
| 注释覆盖 | `tools/check_comments.py` 实测：`wreader/` + `tests/` 有 **3204** 条语句上方没有紧邻注释行（2026-09-25 是 2771 → ㉜ 后 3099；口径与处置见待办 #1） |
...
| 开发期校验 | `tools/` 全绿：文档锚点 OK、数字对拍 ALL OK、注释报告 3204、折行 40077、绘制 140、鼠标 8 项、成就 19 项、配色干净退出（后三项本会话未复跑，见 `activeContext.md` ㉝） |
| 包规模 | `wreader/` **12** 个 `.py` / **9,803** 行 + `data/achievements.json`（348 行 / 48 条） |
...
### 配置
- `settings.toml`，**4 个 section / 18 个键**（`[reader]` 11 键：2026-09-26 新增
  `auto_scroll_interval`=5.0 / `auto_scroll_step`=1），由 `SCHEMA` 单一事实来源驱动（默认值、类型、写序、行尾注释）。
...
- **618** 项自动化测试（全离线、每测试独立 `tmp_path`；分文件计数见 `techContext.md`）。
...
- `wreader/` 12 个 + `tests/` 11 个 Python 文件在 2026-09 大幅补过一轮口语化中文注释；
  ⚠️ 但**严格口径下没做到 100%**（`tools/check_comments.py` 实测还有 **3204** 条语句上方没有紧邻注释行），
...
### 高优先级
1. **决定「注释覆盖率」怎么处理**（2026-09-22 新发现，至今未拍板；2026-09-26 复测 **3204**）：
   严格按「每条逻辑语句上方一行注释」测，`wreader/` + `tests/` 还有这么多条不满足
   （最多的是 `test_reader.py` 664、`reader.py` 509、`achievements.py` 256、`library.py` 233、
   `test_library.py` 229；2026-09-25 是 2771，㉜ 的搬运功能 + 本会话的自动翻页共添了 400 多条）。
   三个选项：
