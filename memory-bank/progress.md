...
| 测试 | **522 passed**，全离线、不碰真实数据，约 3~25 秒 |
| 类型检查 | `npx pyright` → **0 errors, 0 warnings, 0 informations**（`wreader/`、`tests/`、`tools/` 都纳入） |
| 注释覆盖 | `tools/check_comments.py` 实测：`wreader/` + `tests/` 仍有 **2142** 条语句上方没有紧邻注释行（口径与处置见待办 #4） |
...
| 开发期校验 | `tools/` 全绿：文档锚点 OK、数字对拍 ALL OK、折行 40077、绘制 420、鼠标 7 项全过 |
...
- **翻页保留 3 行上下文**（2026-09-22 新增）：键盘翻页（`j`/空格/`PageDown` 与 `k`/`PageUp`）
  的步长减去 `reader.page_overlap`（默认 3）行，所以上一屏末尾 3 行会留在新屏幕顶部（往回翻则留在底部），
  整页翻也不会一下子跳断。设 `0` = 关闭重叠、翻页就是整屏跳转。
- **鼠标滚轮 / 触摸拖动逐行滚动**（2026-09-22 新增）：滚轮一格 1 行（`reader.wheel_scroll_step`）、
  ...
  ⚠️ 逐行滚动不经过 `step_lines`，所以**不受翻页重叠影响**（它本来就是一行一行走，上下文天然连着）。
...
### 工程质量
- 474 → **522** 项自动化测试（全离线、每测试独立 `tmp_path`）。
...
- `wreader/` 8 个 + `tests/` 8 个 Python 文件在 2026-09 大幅补过一轮口语化中文注释；
  ⚠️ 但**严格口径下没做到 100%**（`tools/check_comments.py` 实测还有 2142 条语句上方没有紧邻注释行），
...
4. **决定「注释覆盖率」怎么处理**（2026-09-22 新发现，需要拍板）：
   ...
   (c) 全量补齐 2142 处 —— 工作量极大，且大量只是给 `return` / `assert` 补一句废话，不建议。
...
| **2026-09-22** | 把"注释全覆盖"从**事实**改成**目标**（口径：一段逻辑配一段中文注释） | 严格测量发现 `wreader/`+`tests/` 还有 2054 条差距（后随代码增加涨到 **2142**）；早先的 `TOTAL: 0` 是脚本 bug 造成的假绿，留着旧说法会误导下个会话 |
