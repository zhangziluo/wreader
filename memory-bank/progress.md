...
### 阅读器（curses）
- 按键：`q Q Ctrl-C` 退出、`j/空格/回车/↓/PageDown` 下翻、`k/↑/PageUp` 上翻、`a` 自动翻页开关
  （`>`/`<` 调速）、`g` 跳行、`G` 到末尾、`Tab` 目录浮层、`[` `]` 章节跳转、`/` 搜索、
  `n` 下一个命中、`b` 书签、`?` 帮助页。**没有**视图切换 / 翻译 / 查词 / 笔记键（2026-09-25 删除）。
...
- **自动翻页 / 免手翻**（2026-09-26 新增）：按 `a` 打开，此后每 `reader.auto_scroll_interval` 秒
  （默认 5）自动往前走 `reader.auto_scroll_step` **屏幕行**（默认 1）；`>` / `+` / `=` 加速（÷2）、
  `<` / `-` / `_` 减速（×2），间隔夹在 **0.5s ~ 600s**。打开时底部提示栏改成
  `自动翻页中 · 每 5 秒 1 行（12 行/分钟） · a 暂停 > 加速 < 减速`。
  三个行为约定：**任何按键 / 滚轮 / 窗口变化都把下一拍推后一整间隔**（正在打字的人不该被抢页）；
  **到书末自动关掉并提示** `已经读到全书末尾，自动翻页已停`；**照常计进度与时长，但不喂**
  `page_streak` 与按键类成就（那是给手动阅读的）。实现见 `systemPatterns.md` 模式 #13。
- **自动翻页的防作弊校验**（2026-09-26 新增，`systemPatterns.md` 模式 #14）：连续自动翻页
  `reader.auto_scroll_check_minutes` 分钟（默认 **10**，可填小数，`0` = 关掉）后，屏幕中间弹一道
  **100 以内加减乘除**的四选一题（`1) 62` 这样带序号，正确选项的位置每次都变）。
  按 `1`~`4` 选**任意**一个 → 自动翻页继续（**答错也放过**，只是在确认键盘前有人），并在提示栏报出
  正确答案；**没作答**（`Esc` / 别的键 / 超过 `reader.auto_scroll_check_seconds` 秒，默认 30）
  → 停掉自动翻页并提示 `校验题没有作答，自动翻页已停（按 a 重新开始）`。
  计时口径：从**打开自动翻页**起算，读者按键 / 滚轮 / 改窗口**不会**把它推后（推后的只有下一拍翻页）。
...
### 配置
- `settings.toml`，**4 个 section / 20 个键**（`[reader]` 13 键：2026-09-26 新增
  `auto_scroll_interval`=5.0 / `auto_scroll_step`=1 / `auto_scroll_check_minutes`=10.0 /
  `auto_scroll_check_seconds`=30），由 `SCHEMA` 单一事实来源驱动（默认值、类型、写序、行尾注释）。
...
1. **决定「注释覆盖率」怎么处理**（2026-09-22 新发现，至今未拍板；2026-09-26 复测 **3403**）：
   ...
   （最多的是 `test_reader.py` 777、`reader.py` 585、`achievements.py` 256、`library.py` 233、
...
| 自动翻页**不参与**按键类成就与连续翻页计数 | 开着 `a` 读书时「翻页永动机」「手速达人」这类成就不会推进 | 有意为之：那些衡量的是**手动**操作，让定时器去刷等于作弊（2026-09-26）；进度与时长照常累加 |
| 自动翻页**只在本次阅读里有效** | 退出阅读器即关闭，也不跨会话记忆 | 会话开始时一律是关的 —— 免得下次打开书就自己动起来（2026-09-26） |
| 校验题**答错也放过**、任意选项都算作答 | 想连续无人值守翻页的人只要盲按 `1` 就能通过 | 这条校验考的是「**有人在**」，不是算术水平（用户 2026-09-26 明确选的口径）；真要紧的是「没人作答就停」那一半 |
| 校验计时**不因读者操作而重置** | 一边读一边按键十分钟，照样会弹题 | 量与"自动翻页连着开了多久"，按键只推后下一拍翻页（否则敲个键就能永久躲过校验）；嫌烦就设 `auto_scroll_check_minutes 0` |
...
| **2026-09-25** | **删除翻译 / 生词本 / 笔记三个功能**（`translator.py`、`translate/`（8 文件）、`vocab.py`、`notes.py` 及全部键位、CLI 子命令、配置段、专用校验脚本、测试） | 用户明确要求"去掉翻译、生词本、笔记相关功能"。一次性删净比留死代码好：留下 `notes.py` 却不给入口，下个会话只会以为它还在用。实测规模：整次提交 **-11,240 / +1,163 行**、47 个文件（`git show --stat`，含测试、文档与记忆库） |
...
| **2026-09-26** | 自动翻页的步长用**屏幕行**（`auto_scroll_step`，默认 1），既不整屏也不源行 | 与 `next_page` / `scroll` 同一套单位：长段落被终端折成多行时**一行一行**走过，不会被整段跳过；默认 1 行是「好跟、不容易看漏」，想快就调间隔 × 步长 |
| **2026-09-26** | **任何按键 / 滚轮 / 窗口变化都把下一拍推后一整间隔**（`defer_auto_scroll`），而不是「一按键就暂停」 | 手动翻页 / 搜索 / 调窗口都是读者在主动操作；推后一拍既不会「刚翻半屏又被抢走」，也不必为了继续自动而重新按 `a`。真想停就再按一次 `a` |
| **2026-09-26** | 到书末**自动关掉并提示**，不平移到开头、不静默空转 | 免手模式最怕「屏幕上什么都没发生，但模式还亮着」。提示一句 `已经读到全书末尾，自动翻页已停`，用户才知道该换下一本 |
| **2026-09-26** | 自动翻页**照常计进度与时长**，但**不喂** `page_streak` / 按键类成就 | 时长和行区间代表「真的读了」，必须算；而「连续翻页」「手速」衡量的正是手动操作，让定时器刷等于作弊 —— 与「字数按行区间去重」同一个诚实口径 |
| **2026-09-26** | 间隔夹在 **0.5s ~ 600s**、`>`/`<` 各 ×2/÷2 调，并落进 `SCHEMA`（默认 5.0 / 1） | 比 0.5 秒快到眼睛跟不上，比 10 分钟还不如自己按 `j`；而「速度可配」才不用每次进阅读器按 `>`/`<`（项目约定：阅读行为不写魔数） |
| **2026-09-26** | 开始自动翻页的**入口只有一个 `a` 键**，不做独立「朗读 / 自动滚动」模式或 CLI 参数 | 免手翻的触发点就在「正在读」的那一刻；把它做成模式开关会多一堆「模式 × 视图」组合。步长 / 间隔交给设置项，键位只留开关与调速 |
| **2026-09-26** | 加**防作弊校验**：连续自动翻页 10 分钟弹一道 100 以内加减乘除的四选一题，选一个选项才继续 | 用户要求「防作弊」：免手翻最容易被拿来挂着刷时长 / 成就。选四选一而不是输入答案，是为了手机上也能一个键作答（Termux 键盘小）；题目每次随机（运算符、操作数、选项顺序）挡住脚本背答案 |
| **2026-09-26** | **任意选项都算作答**（答错也继续），只有「没作答」才停 | 用户拍板：这道题的目的是确认**键盘前有人**，不是考算术。判分反而会让"手滑点错"变成惩罚；提示栏里报出正确答案，答错的人也能看一眼 |
| **2026-09-26** | 停下自动翻页的出口**三个**：`Esc` / 任意非选项键 / 倒计时（默认 30 秒）走完 | `Esc` 是用户明确要求的"我不管了"；别的键按"没选择"处理（原话：「如果用户没有选择，自动翻页结束」）；倒计时则保证「人走了但终端没人按键」这种情况也会停 |
