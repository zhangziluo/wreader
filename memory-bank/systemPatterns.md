...
41. **验证「阅读中某一刻的位置」不能只看 `library.json`**（2026-09-26 实测）：位置只在**退出**
    （`open_reader` → `save_position`）与每 `reader.auto_save_interval` 秒（默认 **60**）落盘，
    所以 pty 脚本跑到一半去读 `library.json`，拿到的是**开书时那份存档** —— 第一版自动翻页的
