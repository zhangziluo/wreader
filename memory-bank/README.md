...
- **全量复核基线快照（2026-09-23，最新）**：**832** 项测试 / `pyright` **0 errors, 0 warnings** /
  **45** 个 `.py`（wreader 21 + tests 14 + tools 10）；`tools/` 的 **10** 个校验脚本全绿
  （含真 pty 的 `verify_notes.py`、`verify_achievements.py` **19 项**）。
  ⚠️ 注释覆盖**不是** 100%：严格口径下 `wreader/` + `tests/` 有 **4513** 条语句上方没有紧邻注释行。
  以上都是**实测值**，不是估算。
- **更早的基线（2026-09-23 成就 Phase 1 + 笔记 Phase 3，仅作对照）**：727 项测试 / 41 个 `.py` /
  注释缺口 3819 / `tools/` 9 个脚本。数字已过期，别再引用。
- **更早的基线（2026-09-22，仅作对照）**：514 项测试 / 23 个 `.py`（wreader 8 + tests 8 + tools 7）/
  40 个跟踪文件；`tools/` 的 7 个校验脚本。数字已过期，别再引用。
