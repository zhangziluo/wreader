...
| `systemPatterns.md` | 分层架构、模块职责表、9 个关键设计模式、关键调用链、必踩的坑 | 改代码之前 |
...
- **全量复核基线快照（2026-09-23，最新）**：**727** 项测试 / `pyright` **0 errors, 0 warnings** /
  **41** 个 `.py`（wreader 20 + tests 12 + tools 9）/ **59** 个跟踪文件；
  `tools/` 的 **9** 个校验脚本全绿（含真 pty 的 `verify_notes.py` **20 项**）。
  ⚠️ 注释覆盖**不是** 100%：严格口径下 `wreader/` + `tests/` 还有 **3819** 条语句上方没有紧邻注释行。
  以上都是**实测值**，不是估算。
