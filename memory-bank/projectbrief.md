...
| 规模（2026-09-26 实测） | `wreader/` **12** 个 `.py`（**10,182** 行）+ `data/achievements.json`（348 行 / 48 条）；`tests/` **10** 个测试文件 + `conftest.py`，**633** 项测试；`tools/` **8** 个校验脚本 |
...
3. **阅读**：`werd read <book_id>` 全屏 curses 分页器；位置、书签、本次时长落库，随时续读。
   ...
   **自动翻页 / 免手翻**（按 `a` 让它每几秒自己往下走，`>`/`<` 调速，
   ...
   连续自动翻页 `reader.auto_scroll_check_minutes` 分钟（默认 10）会弹一道
   100 以内加减乘除的**四选一**题，按 `1`~`4` 选一个才继续，没人作答就停），
   以及 `?` 帮助页、意外中断后的恢复提示。
...
6. **配置**：`~/.wreader/settings.toml`（**4 个 section / 20 个键**）；
